...
class ArtistUseCase @Inject constructor(private val artistsRepository: ArtistsRepository) {
    suspend operator fun invoke(artistId: String, fetchRemote: Boolean = true) =
        artistsRepository.getArtist(artistId = artistId, fetchRemote = fetchRemote)
}
