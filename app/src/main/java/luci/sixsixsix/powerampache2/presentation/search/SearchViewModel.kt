...
class SearchViewModel @Inject constructor(
    ...
    private fun fetchGenres() = viewModelScope.launch {
        musicRepository.getGenres(fetchRemote = true).collect { result ->
            when (result) {
                is Resource.Success -> result.data?.let { genres ->
    ...
    private fun search() = viewModelScope.launch {
        ...
        val searchSongsDeferred = async { searchSongs() }
        val searchAlbumsDeferred = async { searchAlbums() }
        val searchPlaylistsDeferred = async { searchPlaylists() }
        val searchArtistsDeferred = async { searchArtists() }
    ...
    fun onEvent(event: SearchViewEvent) {
        when (event) {
            ...
            is SearchViewEvent.OnGenreSelected -> viewModelScope.launch {
